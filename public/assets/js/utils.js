function showModalFilter(){
  let $filter = $('.filter__container').first().clone();
  $filter.addClass('show-in-modal');
  $filter.find('.form-select').addClass('select__bordered'); 
  $filter.find('.btn__simple').addClass('border'); 
  $('#modalFilterBody').append($filter);
  $('.filter__container').first().empty();
...
function showMessage(alertClass, message) {
  ...
  if(alertClass === 'alert-info'){
    style = { background: "#00f28b", color: '#1b1b1b' };
  }
